...
// Cast the imported module to any so TypeScript doesn't complain about missing properties
const AnySDK: any = PhoenixSDK;
const Client = AnySDK.Client ?? AnySDK.default?.Client;

const MARKET = new PublicKey(
  process.env.PHOENIX_MARKET ?? "4DoNfFBfF7UokCC2FQzriy7yHK6DY6NVdYpuekQ5pRgg"
);
const RPC =
...
(async () => {
  ...
  const client =
    typeof anyClient.create === "function"
      ? await anyClient.create(conn)
      : new anyClient(conn);
  ...
  try {
    ...
    console.log("getL2 error:", String(e));
  }
  ...
  try {
    ...
    console.log("getBbo error:", String(e));
  }
  ...
  try {
    if (has("refreshMarket")) {
      ...
      console.log(
        "refreshMarket(false) bids_len/asks_len:",
        ob.bids?.length ?? 0,
        ob.asks?.length ?? 0
      );
    }
  } catch (e) {
    console.log("refreshMarket error:", String(e));
  }
})();
