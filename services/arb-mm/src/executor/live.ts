...
// Live executor with preflight guards, dynamic/fixed tips, optional Jito,
// and ML lifecycle logging. Keeps payer === user authority to avoid
...
export type ExecPayload = {
  path: ExecPath;
  size_base: number; // human units
  buy_px?: number;   // indicative buy px (quote/base)
  sell_px?: number;  // indicative sell px (quote/base)
...
export class LiveExecutor {
  ...
  private submitMode = String(process.env.SUBMIT_MODE ?? "rpc"); // "rpc" | "jito"
  ...
  // Use the key that will actually sign as both payer AND user authority.
  private resolvePayerPk(): PublicKey {
    const anyA: any = this.a as any;
    return (
      anyA.signer?.publicKey ||
      anyA.wallet?.publicKey ||
      this.a.owner
    );
  }
  ...
  private async signTx(tx: VersionedTransaction) {
    const anyA: any = this.a as any;
    try {
      if (typeof anyA.sign === "function") {
        await anyA.sign(tx);
        return;
      }
      if (anyA.signer?.secretKey) {
        tx.sign([anyA.signer]);
        return;
      }
  ...
  private computeBudgetIxs(units: number) {
    ...
    return { ixs, lamports, microLamports, estUnits };
  }

  async maybeExecute(exec: ExecPayload) {
    // Global guards
    if (killSwitchActive()) {
      logger.log("guard_violation", { type: "kill_switch_active" });
      return;
    }
    const g = guardCheck({
      pathId: exec.path,
    ...
    if (!g.ok) {
      logger.log("guard_violation", { type: g.reason, value: g.value, limit: g.limit });
      return;
    }

    const payerPk = this.resolvePayerPk();

    // Wallet/ATA preflight against the payer/authority key
    try {
    ...
    if (!this.phxReady) {
      logger.log("submit_error", {
        where: "phoenix_not_ready",
        error: "Phoenix hedge unavailable; skipping to avoid net exposure",
      });
      return;
    }
    ...
    try {
      // ---- Build Raydium swap with payerPk as user authority ----
      const baseIn = exec.path === "PHX->AMM";
      const amountInBaseAtoms = BigInt(Math.round(exec.size_base * 1e9)); // SOL 9dp

      const ray = buildRaydiumCpmmSwapIx({
        user: payerPk,
        baseIn,
      ...
      if (!ray.ok) {
        noteError();
        logger.log("submit_error", { where: "raydium_build", error: ray.reason || "unknown" });
        emitMl("error", { where: "raydium_build", error: ray.reason || "unknown" });
        return;
      }
      ...
      // Log submit now that we know the IX count
      logger.log("submitted_tx", {
        path: exec.path,
        size_base: exec.size_base,
        buy_px: exec.buy_px,
      ...
      const msg = new TransactionMessage({
        payerKey: payerPk, // payer == user authority
      ...
      if (this.submitMode.toLowerCase() === "jito") {
        try {
          ...
          if (res?.ok) {
            ...
            logger.log("jito_bundle_ok", { bundle: res.bundleId, signature: sig || undefined });
          } else {
      ...
      if (!sig) {
        sig = await this.conn.sendTransaction(tx, { skipPreflight: false, maxRetries: 3 });
      }
      ...
      // Typical: “Transaction signature verification failure”
      noteConsecutiveResult(exec.path, false);
      noteError();

      let logs: string[] | undefined =
        e?.logs || e?.data?.logs || e?.value?.logs;

      // Try to simulate for logs if we still don't have any
      if (!logs && tx) {
        try {
